...
public static class EnumerableExtensions
{
    ...
    public static IEnumerable<U> WhereNotNull<T, U>(this IEnumerable<T> source, Func<T, U?> func)
        where U : struct
    {
        return source
            .Select(func)
    ...
    {
        public bool None(Func<T, bool> pred)
        {
        ...
        public IEnumerable<Indexed<T>> WithIndex()
        {
            return source.Select((x, i) => new Indexed<T>(i, x));
        }

        public bool MoreThanOneItem()
        {
        ...
        public (T First, T Second)? MaybeJustTwoItems()
        {
        ...
        public (T First, T Second) JustTwoItems()
        {
            if (source is IList<T> l)
            {
                int count = l.Count;
        ...
        public (T A, T B) At2(int indexA, int indexB)
        {
            if (indexA >= indexB)
            {
