...
- **Git identity:** never override git identity (no `-c user.name/email`, no `GIT_AUTHOR_*`/`GIT_COMMITTER_*` env); commits must use the repo's configured identity; model attribution goes ONLY in the `Generated-by:` trailer.
